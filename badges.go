...
func (s *badges) Create(target_org_resource_ids []string, badge_data map[string]string) error {
	...
	_, err := s.client.makeRequest(http.MethodPost, "/v2/public/badges/create", BadgeRequest{Org_Resource_IDs: target_org_resource_ids, Badges: createBadgesFromMap(badge_data)}, nil)
	if err != nil {
		return err
	}
...
func (s *badges) Update(org_resource_id string, badges map[string]string) error {
	...
	_, err := s.client.makeRequest(http.MethodPost, fmt.Sprintf("/v2/public/badges/%s/update", org_resource_id), createBadgesFromMap(badges), nil)
	if err != nil {
		return err
	}
...
func (s *badges) Delete(target_org_resource_ids []string, badges map[string]string) error {
	...
	_, err := s.client.makeRequest(http.MethodPost, "/v2/public/badges/delete", BadgeRequest{Org_Resource_IDs: target_org_resource_ids, Badges: createBadgesFromMap(badges)}, nil)
	if err != nil {
		return err
	}
...
func (s *badges) ListCloudsWithBadges(badges map[string]string) ([]CloudBadges, error) {
	...
	body, err := s.client.makeRequest(http.MethodPost, "/v2/public/badge/clouds/list", badges, nil)
	if err != nil {
...
func (s *badges) ListResourceBadges(org_resource_id string) ([]Badge, error) {
	...
	body, err := s.client.makeRequest(http.MethodPost, fmt.Sprintf("/v2/public/badges/%s/list", org_resource_id), nil, nil)
	if err != nil {
		return []Badge{}, err
	}
...
func (s *badges) ListResourceBadgeCount(resource_ids []string) (Resource_Count, error) {
	...
	body, err := s.client.makeRequest(http.MethodPost, "/v2/public/badges/count", BadgeResourceCountRequest{Resource_IDs: resource_ids}, nil)
	if err != nil {
